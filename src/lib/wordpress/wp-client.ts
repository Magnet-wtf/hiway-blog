...
const API_BASE_URL = process.env.WORDPRESS_API_BASE_URL ?? 'https://hiway.blog/wp-json';
...
export default class WpClient {
    ...
    async postComment(postId: number, name: string, email: string, content: string): Promise<void> {
    ...
    private getHeaders() {
        const auth = this.createWordpressBasicAuthHeader(this.username, this.password);
